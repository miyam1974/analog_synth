...
| **State**       | パラメータ保持、プリセット入出力、UI 可視化用の再生ヘッド  |
...
`MainComponent` はヘッダー（タイトル）＋ `SynthEditor` ＋ 仮想キーボード（`MidiKeyboardComponent`、MIDI 36〜96）で構成。ウィンドウ初期サイズは **1080×680**（`MainWindow::centreWithSize`）。
...
| **PRESET** / **SAVE** / **LOAD** | プリセット選択・保存・読込                        |
| **MASTER**                       | 出力レベル                                |
...
| パネル    | 内容                                                         |
| ------ | ---------------------------------------------------------- |
| OSC    | OSC1 / OSC2 各 4 波形、TUNE / FINE / DET2（RESET 付き）            |
| MIXER  | OSC1 / OSC2 / SUB / NOISE レベル、Sub オクターブ、Glide、V-A / V-F    |
...
- コントロール hover でフッターステータス行に説明表示

### コールバック

`SynthEditor` は `std::function` で `MainComponent` にイベントを返す（MIDI 選択、Panic、Mono、プリセット操作）。

モジュール幅（`SynthEditor::resized`）: **OSC 21% / MIXER 16% / FILTER 26% / AMP 18%**、残りを LFO
...
- JSON キー: LFO1 は `lfoRateHz` 等、LFO2 は `lfo2RateHz` / `lfo2Depth` / `lfo2To`*（旧プリセットは LFO1 のみ）
- `captureCurrentParameters` / `applyParametersFromVar` で `SynthParameters` と JSON を相互変換
- プリセット変更時 → `onParametersChanged` → `SynthEditor::refreshUIFromParameters`
...
target_sources → Main, SynthEditor, PresetManager, SynthVoice,
                 EnvelopePlayhead, SynthSound, UI/*（6 ファイル）
