...
1. [Releases](https://github.com/miyam1974/analog_synth/releases) を開き、最新のバージョン（例: `v0.3.0`）を選ぶ
...
[`.github/workflows/release.yml`](.github/workflows/release.yml) が、`v*` 形式のタグ（例: `v0.3.0`）の push を検知したときだけ起動します。

### サーバー上で行われる処理

1. リポジトリのソースをチェックアウト
2. CMake で Visual Studio 2022 / x64 の Release ビルドを実行
...
1. 必要なら `Main.cpp` の `getApplicationVersion()` を更新する（表示バージョンとタグを揃える）
2. 変更を `main` に commit / push する
3. タグを付けて push する（例: `git tag v0.3.0` → `git push origin v0.3.0`）
